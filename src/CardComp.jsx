...
const CardComp = () => {
    ...
    const totalCartQuantity = useSelector((state) =>
        state.blogs.reduce((total, item) => total + (item.quantity || 0), 0)
    );

    // Selecting total price of items in the cart from Redux store
    const totalPrice = useSelector((state) =>
        state.blogs.reduce((total, item) => total + item.price * (item.quantity || 0), 0)
    );
                                        ...
                                        <div className="card-body">
                                            ...
                                            <div className="d-flex justify-content-between ">
                                                ...
                                                <div className="d-flex align-items-center">
                                                    <button className="mx-2 quantity-btn p-2"><i className="fa-solid fa-plus" onClick={() => handleIncrease(item.id, item.quantity || 0)}></i></button>
                                                    <h6 className="mx-2"> {quantities[item.id]}</h6>
                                                    <button className="mx-2 quantity-btn p-2"><i className="fa-solid fa-minus" onClick={() => handleDecrease(item.id, item.quantity || 0)}></i></button>
                                                </div>
                                            </div>
                                            {/* Star rating */}
